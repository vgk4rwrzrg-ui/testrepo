...
def assemble_markdown(job: ReportJob) -> str:
    ...
    # Audit & provenance footer (same data as the themed documents).
    from .documents import spec_from_report
    meta = spec_from_report(job).get("meta") or {}
    model = meta.get("model_info") or {}
    parts += ["", "---", "", "## Audit & Provenance", "",
              "_This report was generated by AI._",
              f"- AI engine: {model.get('provider') or ''} "
              f"{model.get('model') or model.get('mode', '')}".rstrip(),
              f"- Requested by: {meta.get('requested_by', '')}",
              f"- Generated at: {meta.get('generated_at', '')}"]
    for src in meta.get("sources") or []:
        parts.append(f"- `{src.get('table')}`: {src.get('reads', 0)} reads "
                     f"({src.get('allowed', 0)} allowed, "
                     f"{src.get('denied', 0)} denied)")
    parts.append("- All table reads were policy-checked and recorded in "
                 "the Table access audit log.")
    return "\n".join(parts)
