...
Under each answer there is a collapsible **🔍 Sources & audit trail**
section. Expand it to see exactly which tables were searched, how many
records each returned, and the AI engine/model that produced the answer.
Every read the bot performs is policy-checked and recorded in the
administrators' Table access audit log.
...
Every document (and every report download, including markdown) ends with an
**Audit & Provenance** section: a "Generated by AI" statement, the AI
provider/model used, who requested it and when, and a table of the data
sources that were read (with allowed/denied counts for reports). The title
page/slide subtitle also carries the *Generated by AI (model)* stamp — so a
file forwarded outside the team is always identifiable as AI-generated.

## 3a. Calculations across fields (and tables)

Ask for formulas in plain language — *"divide hours by budget and multiply
by headcount for each open requisition"*. When an LLM is connected, it runs
the formula **in the database** through the `compute_data` tool rather than
doing arithmetic itself, so the numbers are exact over all matching rows,
division by zero shows as *null*, and the formula **and** its result are in
the audit log. Formulas can reach related tables (e.g. a requisition's
program budget) and combine totals from unrelated tables — provided your
groups may see every table and field involved.
